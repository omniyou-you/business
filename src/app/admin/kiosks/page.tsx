...
import { 
  Cpu, Plus, Key, Check, ShieldAlert, Copy, RefreshCw, X, HardDrive, Wifi
} from "lucide-react";
...
interface KioskNode {
  ...
  location: string;
  api_key: string;
  status: "ONLINE" | "MAINTENANCE" | "REVOKED";
  last_ping: string;
}
...
export default function AdminKiosksPage() {
  const [nodes, setNodes] = useState<KioskNode[]>([
    {
      id: "node-1",
      machine_id: "KIOSK-MALL-01",
      location: "Central Mall, 1st Floor",
      api_key: "kiosk_live_sec_89421098347192847",
      status: "ONLINE",
      last_ping: "Just now",
    },
    {
      id: "node-2",
      machine_id: "KIOSK-CAMPUS-02",
      location: "University Student Library",
      api_key: "kiosk_live_sec_37192847984210984",
      status: "ONLINE",
  ...
  const handleAddKiosk = (e: React.FormEvent) => {
    ...
    const newKey = `kiosk_live_sec_${Math.random().toString(36).substring(2)}${Date.now()}`;
    const newNode: KioskNode = {
      id: `node-${Date.now()}`,
      machine_id: newMachineId.toUpperCase(),
  ...
  const toggleRevoke = (id: string) => {
    setNodes(
      nodes.map((n) =>
        n.id === id ? { ...n, status: n.status === "REVOKED" ? "ONLINE" : "REVOKED" } : n
      )
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-400">Monitor physical kiosk pings, register new hardware nodes, and manage API Keys.</p>
        </div>
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5 space-y-4 backdrop-blur-sm">
        <div className="flex items-center justify-between border-b border-zinc-800/60 pb-3">
          <h3 className="font-bold text-white text-sm">Active Registered Kiosks ({nodes.length})</h3>
          <span className="text-[11px] font-mono bg-emerald-500/10 text-emerald-400 px-2.5 py-1 rounded-full border border-emerald-500/20">
            ● {nodes.filter((n) => n.status === "ONLINE").length} Nodes Healthy
          </span>
        </div>
          ...
          <table className="w-full text-left text-xs text-zinc-300">
            <thead className="bg-zinc-900/60 font-semibold text-[11px] text-zinc-400 border-b border-zinc-800 uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-3.5">Ping Status</th>
                ...
                <tr key={node.id} className="hover:bg-zinc-900/40 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    >
                      ...
                      {node.status} ({node.last_ping})
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <button
                      onClick={() => toggleRevoke(node.id)}
          ...
          <div className="bg-zinc-950 border border-zinc-800 p-6 rounded-2xl max-w-md w-full space-y-5 relative shadow-2xl">
            ...
            <div className="flex items-center gap-3 border-b border-zinc-800 pb-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-zinc-500">Generate a new secret API Key for a physical kiosk machine.</p>
              </div>
            </div>

            <form onSubmit={handleAddKiosk} className="space-y-4 text-xs">
              <div className="space-y-1">
                <label className="text-zinc-300 font-medium">Machine ID (e.g. KIOSK-MALL-03)</label>
                <input
                  type="text"
                  value={newMachineId}
                  onChange={(e) => setNewMachineId(e.target.value)}
                  placeholder="KIOSK-MALL-03"
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="Downtown Station, Gate 4"
                  className="w-full h-10 rounded-lg bg-zinc-900 border border-zinc-800 px-3 text-xs text-white focus:outline-none focus:border-blue-500"
                  required
                />
              </div>
              ...
              <div className="pt-2 flex gap-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2.5 rounded-lg glow-button text-white font-bold text-xs"
                >
                  Generate Key & Save
                </button>
              </div>
